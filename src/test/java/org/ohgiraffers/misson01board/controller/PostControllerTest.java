...
/** 통합테스트 & 단위테스트
 *  통합테스트:
 *  모튤을 통합하는 과정에서 모듈 간의 호환성을 확인하기 위해 수행되는 테스트
 *  통합테스트는 API의 모든 과정이 올바르게 동작하는지 확인하는 것
 *
 *  단위(유닛)테스트 :
 *  하나의 모듈을 기준을 독립적으로 진행되는 가장 작은 단위의 테스트 (작은 단위 = 함수,메소드)
...
public class PostControllerTest {
    /**
     * @AutoWired 의존성 주입(DI)을 할 때, 사용하는 필요한 의존 객체 타입이 해당하는 빈을 찾아 주입한다.
     * 의존성 : 하나의 코드가 다른 코드에 의존하는 상태를 뜻한다.
     */
    @Autowired
    MockMvc mockMvc;
    /**
     * @MockBean 스프링의 의존성 주입기능을 사용해 테스트 실행 시 실제 빈 대신에 Mockito로 생성된
    ...
    /**
     * @ObjectMapper 특정 객체를 json 형태로 바꾸기이위해 사용한다.
     */
    @Autowired
...
//                .andDo(print());
    ...
    @Test
    @DisplayName("게시글 전체 조회 페이지 단위 테스트")
    void readAll_post_test() throws Exception{
        ...
        mockMvc.perform(get("/api/v1/posts"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].postId").value(response.getPostId()))
                .andExpect(jsonPath("$.content[0].title").value(response.getTitle()))
                .andExpect(jsonPath("$.content[0].content").value(response.getContent()))
                .andDo(print());
    }
}